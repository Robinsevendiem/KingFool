...
- 策略回测：支持动量窗口、熔断阈值、缓冲阈值、Alpha 因子、暴跌过滤等参数组合。
...
- `data/reference/`：原始策略持仓与调仓对照文件。
- `analysis_results/`：历史分析导出结果，不影响主应用运行。

## 说明

- 仓库已移除代码中的硬编码 `Tushare Token`，适合直接公开到 `GitHub`。
- `scripts/` 中多数文件用于研究或离线分析，不参与 `Streamlit` 线上页面启动。
